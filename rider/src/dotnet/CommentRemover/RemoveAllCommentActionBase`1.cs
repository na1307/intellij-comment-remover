...
public abstract class RemoveAllCommentActionBase<TNode> : RemoveCommentActionBase where TNode : ITreeNode {
    ...
    public sealed override void Execute(IDataContext context, DelegateExecute nextExecute) {
        ...
        var comments = Childrens.OfType<TNode>().Cast<ITreeNode>().Concat(AdditionalNodes).ToArray();
